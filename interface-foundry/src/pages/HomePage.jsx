...
const components = [
  {
    ...
    header: <div className="bg-gray-800 rounded-lg h-full w-full flex items-center justify-center text-gray-400">Görsel Alanı</div>,
  ...
  },
  {
    ...
    header: (
        <img 
            src="https://images.unsplash.com/photo-1549692520-cb01e695d315?q=80&w=800" 
    ...
    <div className="p-4 md:p-8 bg-black min-h-screen">
      ...
      <BentoGrid>
        {components.map((item, i) => (
          <Link to={item.link} key={i} className={item.className} style={{ pointerEvents: item.link === '#' ? 'none' : 'auto' }}>
            <BentoGridItem
              title={item.title}
